...
// Filter for cards visible to Business Associate
const baCards = data.filter(c => {
    return (!c.personas || c.personas.length === 0 || c.personas.includes('generic') || c.personas.includes('business-associate'));
});

console.log(`Analyzing ${baCards.length} cards accessible to Business Associate.`);
...
// Specific BA-only cards balance
const specificCards = data.filter(c => c.personas && c.personas.includes('business-associate') && !c.personas.includes('generic'));
console.log(`\n--- BA Specific Cards (${specificCards.length}) ---`);
